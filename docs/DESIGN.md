...
**Store name (2026-09-10).** "Hooked" is the codename, the repo and the EAS slug. The public store name, the one shoppers see and the one that must be unique per store, is a separate string set in App Store Connect and Play Console; the store name is still to be chosen ("Yarniverse" was dropped on 2026-09-10 because "Knit Stars and Yarniverse" already exists on the App Store; candidates are listed in `docs/QUESTIONS.md` item 22). Verify availability the day the App Store Connect record is created (the record is what proves it) and run a trademark search then; once confirmed, `expo.name` follows it so the icon label matches the listing.
