...
3. **A tribute that ships.** Her real projects and palette are in the game, drawn as illustrations; her name and photos are not. The game is a real commercial release, and the tribute is the game itself and its upside.
...
**Money and accounts (decided 2026-09-10).** Revenue goes to the owner's LLC (reviewer default; the owner asked whether his personal name would do, see `docs/QUESTIONS.md` item 17), and the revenue arrangement with her is written down before phase 9. The Apple Developer team that ships canvass-app is reused; which of the two Google Play accounts (the personal one, or the one canvass-app uses) is still open. Hooked gets its own App Store Connect record, Play app, EAS project and credentials. The Apple paid-apps agreement and a Play merchant profile must be active before phase 7's sandbox purchases. The full product set ships in v1 (six marker packs, the Starter Bundle, the Yarn Bank); prices are set in the stores and RevenueCat. The privacy policy, terms and delete-account pages are committed static HTML served by the API, as canvass-app does; the privacy URL is needed before any *external* TestFlight build, so her phase 6 build goes out to TestFlight's internal testers (no Beta App Review, no URL needed) and the pages are live on the API before phase 9's external build; phases 7 and 9 follow in order. Age rating 4+/Everyone with a declared target audience of 13+, not Made for Kids (reviewer default, owner to confirm).
...
- Credits: "Inspired by a crocheter who is at level 7000." (draft, owner to approve). Her name appears nowhere in the app: the tribute is public, she is not.
...
- Tap the frog on the meter seven times → a hidden level (`id` 7000, `book` 0) with a short thank-you note at the end, written without her name (draft wording in `docs/QUESTIONS.md`, owner to approve). Players finding it is part of the charm.
