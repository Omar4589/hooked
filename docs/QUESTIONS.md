...
### 37. The phase 5 art hand-off: what the designer delivers, who draws the illustrations, how much the brief covers, how the pieces look, and when the animation is commissioned.

**Owner.** Answered 2026-09-11, while getting the brief ready to send: production SVG; AI-generated illustrations against a designer-set style; everything phase 5 needs; soft dimensional pieces; animation later.

**Decision.** (1) The designer delivers **production SVG** for the board rather than sketches the build draws from, so the pieces are the designer's work and not an approximation of it. (2) The **project illustrations are generated** with §16's fixed style phrase, against three test images the designer makes first; the designer sets the style and the generator makes the set match. (3) The brief covers **everything phase 5 needs** — the style sheet, the whole board set, level 1's screens, one illustration, the sound cues — plus every board piece later levels reuse, so nothing is drawn twice. (4) The pieces are **soft dimensional**: a warm side light, a shaded underside and a contact shadow drawn into the vector, so they read round at 34 pt while still recoloring and scaling for free. Flat was the old brief and reads cheaper; fully rendered (modelled and lit per color, exported as PNG) is what Fishdom does and was turned down — it costs the recolor and the scale, and glossy plastic is the wrong material for wool. (5) The **animated showpieces** (confetti, the coin burst, the frog's idle blink) are commissioned **after** level 1's static art is in and moving, so nothing is animated onto art that still changes.

**Recorded in.** docs/design/BRIEF.md (the work order), docs/DESIGN.md §12 and §16

<details><summary>Why it was asked</summary>

The brief was about to be sent to a designer, and four of the five change what is being asked for: the delivery format changes their tooling, the illustration question changes how much of their time this is, the scope changes the quote, and the look changes what they draw. The fifth avoids paying twice for animation. The look question came out of a fair one from the owner — Fishdom's pieces look three-dimensional, and ours would not — which is an art-pipeline difference rather than an engine one: every piece in a match-3 is a flat image with its shading baked in, so the dimensional look was available all along at the cost of a line in the brief.

</details>

### 38. Phase 5's five art calls: is the delivered art the final set, does the yarn bomb replace the ball or ride on it, does a tangle print its layer count, does the frog wiggle at a full meter, and how big is the dial?

**Owner.** Answered 2026-09-12, while the art went into the app and again after playing it on the phone: the delivery is final; the bomb replaces the ball; no numeral; the frog wiggles; the dial is measured.

**Decision.** (1) `apps/mobile/src/art/pieces.js` and `palette.js` **are the designer delivery**, not an interim set someone replaces later. The board set landed as a generated module rather than as 26 SVG files, so the art is code and a redraw is an edit to it; the consequence is that `docs/design/BRIEF.md` deliverable 2 is closed and the brief now asks only for what nothing has drawn — the illustration style, the level card, the win and lose screens, the coaster, the sound cues. (2) The **yarn bomb replaces the ball** instead of overlaying it, because the biggest and loudest of the four specials has to be unmistakable next to the other three, and a dark ball wrapped in five strand colours with a lit fuse is. The consequence is a readability hole the owner accepts and wants a second pass at: a bombed ball still matches by its own colour in the engine, but nothing on the drawing says which colour that is, so a player cannot tell the olive bomb from the rust one. `compose.test.js` pins the behaviour so a later change cannot quietly put a ball back underneath it, and the note back to the designer is in the brief. (3) The **tangle's numeral is dropped**: the three drawn densities — 5, 9 and 14 loops, darker and more opaque each time — carry the layer count, because a digit printed on a piece is a HUD element standing on the board, and the drawing was always meant to say it. The consequence is that the top of the range is unproven: `tangle(1)` and `tangle(2)` are both on a development board (`moths-7x7`, where a buried `x` cell is a two-layer tangle), but no development level places a three-layer tangle or a knot, so `tangle(3)` and `knot()` are drawn and unit-tested and cannot be seen on a phone until Book 1 is authored in phase 6. (4) The **frog wiggles when the meter reads full**, which §16 always asked for and which was unreachable until now: the engine resets the charge inside the same `swap()`/`tap()` call that fills it, so `game.state().meter.charge` is already 0 when the HUD reads it and a dial fed from state could never show a full meter. The move now carries `meterFrames` and the dial samples them against the board clock like every other animation, which is what makes "full" a moment that exists; the engine legitimately reports 11 on a full of 10, so the clamp happens at draw time. (5) The **dial is sized from the measured left column**, 56 to 100 pt, rather than fixed at 100. That column also carries the level, the goal rows and the move counter, and a fixed dial pushes the move counter off the bottom on a four-goal level — the one thing the owner asked never to happen. The consequence is that the dial shrinks first. The art was played on the owner's phone the same day: the 9x9 sandbox held 60 fps at rest and dipped to 43 only during a frog rip. The numbers, and what they do not show, are below.

**Recorded in.** docs/design/BRIEF.md (the work order, deliverables 1–3 and 6), apps/mobile/src/art/ (`pieces.js`, `palette.js`, `compose.js` and `compose.test.js`), apps/mobile/src/game/ (`move.js`, `Meter.jsx`, `timings.js`), apps/mobile/src/screens/PlayScreen.jsx

<details><summary>Why it was asked</summary>

Phase 5 put real art on the board, and these are the five places it had to choose something the spec does not settle. Whether the delivery is final decides what the brief still asks a designer for and what is closed. The bomb and the numeral change what a piece says to the player, and both are cheaper to settle now than after Book 1 is authored against them. The wiggle is in §16 and had never been drawable, because the engine's own state cannot report a full meter. The dial's size is a layout rule the HUD had no room for until the dial moved off the board into the left column, which PlayScreen measures.

**What the devices showed.** Four runs on 2026-09-12, all by the owner. On an iPhone 17 Pro in Expo Go: 60 fps on both threads at rest, dipping to 43 fps during a frog rip (a development bundle); the same scene on a `--no-dev --minify` bundle dipped to 54. On a Moto G Play 2023 — a floor-of-market Android, and from now on the minimum spec this game is measured against — the board holds 59 fps at rest, but opening a 9×9 took 6678 ms in development and **1845 ms in a release APK**. The frog, the double-tap tongue, the dial and the four specials were all confirmed working on both devices, and the left column holds its dial and move counter.

**What the board-open cost is made of**, because it was measured rather than argued. Instrumenting each phase and then changing one variable at a time on the Moto: svg pieces 6678 ms, plain `View` pieces 2988 ms, pieces drawing nothing with every hook still running 2884 ms. So 81 native views cost 104 ms, the ~2,100 svg nodes inside them cost 3690 ms (about 55%), and the rest is React and Reanimated mounting 162 components with worklets. Three suspects were cleared by measurement: the engine (`createGame` is 9 ms in release), the view model (`buildModel`, 2 ms), and svg parsing (~174 ms of the stall, not the seconds first assumed).

**The decision.** §16's PNG fallback stays unused. 1845 ms on the minimum spec is a loading problem before it is a rendering one, and the level card §16 already schedules covers it: a card over the mount turns a freeze into a transition, using art that was being built anyway. Revisit if Book 1's boards are heavier than a sandbox 9×9, or if the level card does not land. The step player hands the renderer plain numbers, so that door stays open at no cost.

Phase 5 is not finished. Still to come: the coaster illustration the win screen shows off, the level cards on Home, blast rings, clear particles, confetti and the coin pile, sounds and haptics.

</details>

### 39. Phase 5 slice 1's five calls: is Book 1 level 1 authored now, is the level card a route or a step inside PlayScreen, does the delivered coaster art ship as it arrived, does the win screen mention the room, and does app.json keep its `expo-font` plugin entry?

**Owner.** Answered 2026-09-13, while the card and the win screen were built and again after playing them on the phone: author level 1 now; the card is a step, not a route; the art ships as delivered, with a re-cut queued; the win screen says nothing about the room; the plugin entry goes.

**Decision.** (1) **Book 1 level 1 is authored now**, rather than in phase 6 with the other fourteen. `packages/levels/levels/book1/001.json` is `docs/LEVELS-BOOK1.md` row 1 verbatim, down to the project id that doc's closing paragraph fixes — "Coaster (olive)", the project `coaster_olive`, a 5×5 round-ish board of 21 playable cells, five colours, 20 moves, `meter: "none"`, one stitch goal over all 21 squares at a single layer, `tutorial: ["swap", "puff"]` and `hard: false` — and `packages/levels/test/book1.test.js` asserts every one of those against the row. Its 100-coin payout is the one field the row cannot settle: the table has no coins column, so that number is authored rather than transcribed and no test holds it to anything. It is authored now because both of slice 1's features need a real level to be true of: the illustration registry keys on the project a *shipped* level carries, and until this file existed `listLevels()` was empty, so a production build had no Play button at all and the win screen could only be reached from a development board. The consequence is a level that is live but not tuned: the random bot wins 13 of seeds 1–20 and loses the other 7, which is **liveness** — both endings reachable, with margin — and not balance. The §11 win rates are measured with the greedy bot, and phase 6's sims set them by tuning moves, colours and blocker density; nothing about level 1 should be read as a balance number yet. `project` is also **required** by `packages/levels/src/schema.js` now, which is the schema catching up with the spec rather than a new rule: §10's worked example already carries `project` in the required block and it is absent from that section's Optional list. All 17 level JSON files in the repo — the six levels and the eleven engine fixtures — already had one, so nothing had to be edited to satisfy it. (2) **The level card is a step inside `PlayScreen`, not the modal route §11 specified.** The card exists because opening a 9×9 board costs a measured 1845 ms on the minimum-spec device (item 38), and the fix is not the card's presentation — it is deferring the board's mount past a painted frame. A route cannot do that: `PlayScreen` measures the arena on one commit and mounts `<Board>` on the next, so the expensive commit lands after `onLayout` returns whatever screen is on top of it. So the card is a sibling in the same subtree, and the deferral is `PlayScreen`'s: it renders the card, measures the arena, and only then schedules a double `requestAnimationFrame` before setting the flag that lets the board mount (one frame is not enough — a `requestAnimationFrame` callback runs *before* the frame it was scheduled for is drawn). `<Board>` reports back through a new `onReady` prop fired from an `onLayout` on its own root, which is the commit that carries the cells and the pieces, rather than from `onState`, which is a passive effect and runs when React commits — the *start* of native mounting, not the end of it. The consequences are all on the card: Play is drawn from the first frame, dimmed and inert, and brightens when the board reports in, because during the mount the JS thread is busy and a live-looking button would take a press and drop it; Back is live from frame one, so a player who opened the wrong level is never held there; and a 4000 ms bail-out flips ready regardless, so a signal that never arrives degrades to a wait and never to a hang. Retry replays the card, because `replayLevel` resets the stack to `[Home, Play]` and this screen mounts again — which is right, since skipping it would give the most-pressed button in a match-3 an uncovered freeze. The departure is recorded in DESIGN.md §11 (Navigation and Screens). (3) **The delivered coaster art ships as it arrived**, and a re-cut is queued with the designer rather than holding the slice for one. Two things are not what `docs/design/BRIEF.md` ordered. The empty state is the finished piece desaturated rather than the line drawing the brief asked for, and it is faint: composited on cream and measured over its own ink it sits 20.8 grey values off the ground on average and 49.9 at its single darkest pixel, against the 18 that `assertGreyscaleSpread()` enforces as the floor for telling two yarns apart — the unfinished state is about as far from the ground as two yarn colours are from each other, which is the bottom of the range and not the middle of it. And both states carry a low-alpha cream wash at their border (alpha 6 to 16 of 255 measured across the two files; the empty one has no fully opaque pixel anywhere, topping out at 221) rather than the hard alpha "PNG on transparent" specifies. The consequence is a placement rule the screens now keep: the illustration is drawn inside the cream card and never on the dark scrim, where that wash haloes. The loss ghost is also drawn at **full** opacity for the same reason — at the 0.55 that "unfinished" suggests it measures about 11.5 grey values off the cream, which is not a fainter picture but no picture. The re-cut lands as the same six filenames, so it costs nothing in code when it arrives. (4) **The win screen says nothing about the room.** The Craft Nook does not exist until phase 6, and a win screen that promises a shelf the player cannot visit is worse than one that says what the level paid. The consequence is that `ResultScreen` is the title, the level's name, the illustration, the score and the coins with the drawn coin beside them, and Retry and Home — and an empty, untouchable effects layer over the card where the confetti and the coin pile mount when they are commissioned (item 37). (5) **The no-op `expo-font` plugin entry is out of `app.json`.** Verified from the plugin's own source (`node_modules/expo-font/plugin/build/withFonts.js`): `withFonts` returns the config untouched when it is passed no props, and adds nothing to either native project without a `fonts` array, so a bare `"expo-font"` string in the plugins list did nothing at all. Fredoka's four faces are loaded at runtime by `useFonts` in `App.jsx`, which is what works in Expo Go and what keeps the .ttf files out of the native projects and off the fingerprint runtime version. The consequence is nothing: `app.json`'s plugins array names `expo-splash-screen` and nothing else, `expo-font` stays a dependency (`~57.0.4`) because `useFonts` comes from it, and the entry never reached a commit — HEAD's `app.json` does not carry it either, so there is no diff to point at.

**Recorded in.** docs/DESIGN.md §11 (Navigation and Screens: the level card is not a route), docs/design/BRIEF.md (deliverable 4 wired, the re-cut ask, the commission trigger), apps/mobile/src/screens/ (`LevelCard.jsx`, `PlayScreen.jsx`, `ResultScreen.jsx`), apps/mobile/src/art/ (`projects.js`, `illustrations.js`, `projects.test.js`), packages/levels/ (`levels/book1/001.json`, `src/schema.js`, `src/registry.js`, `test/book1.test.js`), apps/mobile/app.json (as an absence — see (5))

<details><summary>Why it was asked</summary>

Slice 1 is the first piece of phase 5 that a player rather than a developer would notice, and these are the five places it had to choose something the spec does not settle. Level 1's timing decides whether the illustration and the win screen have anything real to stand on, or whether they are demonstrated on a development board and authored against in phase 6. The card is a departure from a §11 line, which is the kind of thing that has to be written down at the moment it is made or it reads later as an oversight. The art question is money and the designer's time: holding the slice for a re-cut would have stopped the commission the brief gates on level 1's static art moving on a phone. The room question is a promise to the player, and the cheapest time to not make it is before it is made. The font plugin is housekeeping that looked like a native change and was not.

**What the device showed.** One session on 2026-09-13, by the owner, on an iPhone 17 Pro in Expo Go on a development bundle. Every board reports ready between 172 and 298 ms under the card: level 1's 5×5 at 172–298 ms across several opens, the 9×9 sandbox at 242–250, the ring coaster at 269, the charm tail at 206, the moths at 210. The numbers are `PlayScreen`'s own `__DEV__` instrumentation, which logs the mount and then the gap (`[card] board ready at N ms — N ms under the card`). That proves the deferral works: the expensive commit lands under the card rather than in front of the player, on a development bundle, which is the slower of the two.

**What it does not show** is the case the card was built for. The 1845 ms is a Moto G Play 2023 in a release APK (item 38), and **the card has not been run on that device since it was built**. Until it has, the fix is measured only where the problem was smallest, and the bail-out timer is the only thing standing between a board that never reports and a player who cannot start.

Phase 5 is still not finished. Still to come: the drawn blast rings, the clear particles, the confetti and the coin pile, sounds, haptics, the ~5 s idle hint, and Reduce Motion as a feature. Slice 1 fixed Reduce Motion as a *defect* — with the OS setting on, reanimated finishes a reduced animation on its first frame, so the board's move clock was landing on its last instant and running `finishMove` before anything drew — by naming `reduceMotion: ReduceMotion.Never` on every clock and cross-fade (the move clock, the two shuffle fades, the dial's arc, the frog's blink) while leaving the decorative wiggle suppressed. Suppressing overshoot, shake and particles for a player who asked for less motion is the feature, and it is still outstanding. There is no audio file of any kind in the repo and `expo-audio` is not installed.
